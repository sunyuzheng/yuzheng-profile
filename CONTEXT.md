# Yuzheng Sun / 立正 — Core Context

**Public context.** Start here when writing, introducing, or collaborating with Yuzheng. For changing numbers and exact claim boundaries, use [EVIDENCE.md](EVIDENCE.md) and [facts.json](facts.json).

## The Short Version

Yuzheng Sun（孙煜征，立正；既有内容名：课代表立正）has a PhD in Economics from Cornell and has worked across Amazon, Meta, Tencent IEG, and Statsig. He is based in Seattle and now builds courses, communities, enterprise programs, books, public conversations, and tools.

The work looks broad, but it is organized around one defining idea:

> **MAKE WHAT LASTS.**

> **做点真东西。**

Yuzheng believes a person should have the chance to turn judgment and craft into something that enters reality, remains answerable to use and consequence, and can keep creating value. The English line names the time horizon; the Chinese line asks for the first real move. `真东西` is concrete enough for a world to use, test, refuse, improve, or keep choosing; it is not authenticity awarded by the maker. `点` makes beginning possible without weakening that standard. `学点真本事，做点真东西` names his personal career thread: capability beyond credentials and work beyond internal scoreboards. `做出你的代表作` remains the expanded long-term aspiration. AI is where much of this work happens today; it is a powerful medium, not the identity itself.

## The Intellectual Spine Beneath The Idea

Important work may begin in uncertainty rather than conviction; the question and the conviction may emerge through making. Beneath that nonlinearity is a set of relationships rather than equal, sequential steps:

- **world models and the scientific method** try to make causes, constraints, and predictions explicit enough to challenge;
- **craft** updates overlapping understanding through practice, apprenticeship, comparison, and consequence where rules cannot contain the whole problem; tacit and explicit knowing continually translate and correct one another;
- **conviction** is the bridge when some understanding has been internalized deeply enough to support action and bear a cost without becoming immune to evidence;
- **responsible authorship** keeps agency and accountability for the work's purpose, standards, compromises, and evolution;
- **reality tests and time reveals:** neither is an infallible judge, but together they prevent the maker's own explanation from becoming the only proof.

A compact synthesis is:

> **A defining work turns hard-won understanding and lived craft into responsible authorship—and remains answerable to reality, time, and the people it serves.**

These relationships loop. Building can produce understanding; conviction can initiate a work or emerge from it; a failed result can change both the model and the craft. `Understand → internalize → conviction → build` names one important bridge, not a universal origin story. Systems, institutions, maintenance, and distribution may be inputs, part of the craft, or the work itself; compounding is a possible result, not a universal production method.

In the AI era, one useful content shorthand is:

> **AI越会做题，人越要会出题。**

“出题” does not mean writing a better prompt or claiming that AI cannot propose questions. It means deciding what is worth doing, for whom, what counts as useful, and who owns the consequences. AI can help define the problem and make the answer; the answer still has to meet users, organizations, and reality.

A current public research agenda asks what should count as **AI-native work and talent**. `Native` does not mean younger, using more tools, or generating more output. It asks whether a person can return to what the work is for, identify which capabilities, costs, and constraints AI has changed, redesign how the result is produced and verified, and remain responsible for its value and consequences. This is a revisable AI-era category thesis beneath the defining idea, not a replacement personal identity; [DATED_VIEWS.md](DATED_VIEWS.md) owns the fuller definition and update conditions.

## What Is Unusual

No single credential explains Yuzheng's perspective. The useful combination is:

- causal reasoning from economics, joined with responsibility for real product and organizational decisions;
- work across the United States and China, large companies and startups, individual contribution and team leadership;
- the ability to connect technical possibility with user value, incentives, adoption, and distribution;
- experience turning tacit judgment into something another person can understand, practice, inspect, and eventually own;
- repeated feedback from researchers, founders, learners, community members, and enterprise teams;
- a public record of dated views that can be checked against what happened later.

A concise description is:

> a founder and builder who turns judgment and craft into products, systems, and institutions that can keep creating value.

## How The Current Work Fits

| Part | Role |
|---|---|
| Public content and conversations | Name consequential questions, show the standards behind the work, and attract people who want to make work of their own |
| Books and public tools | Preserve ideas and selected working systems so others can inspect them and, where licensed, use them |
| Superlinear Academy | The parent institution and master brand Yuzheng is building around this belief; its free community is the open public entry point |
| AI Builders | A structured learning product within Superlinear Academy; with Yage, it teaches AI principles, building methods, and judgment through work on real problems |
| Stay Superlinear | The paid ongoing membership product within Superlinear Academy, organized around high-value content, expert judgment, activities, tools, and a sustained environment |
| Enterprise programs | Apply the same methods inside real workflows, incentives, risks, and organizational constraints |

The parts reinforce one another, but each should create value on its own. See [ECOSYSTEM.md](ECOSYSTEM.md) for the fuller relationship.

## Founder, Teaching Partner, And Community

- **Yuzheng** is the founder, author, and public steward of the defining idea. Superlinear is the defining work he is pursuing through long-term practice and the institution through which he is making that idea usable by others.
- **Yage** is an important long-term teaching partner and technical authority. His research and engineering judgment give AI Builders, Knowledge Bank, and membership content unusual technical depth.
- **Learners and members** are the protagonists of their own work. They bring realities neither founder nor teacher can own alone: different industries, users, constraints, failures, and results.

These roles reinforce one another without being interchangeable. The founder gives the institution a belief and standard; collaborators make its products deeper; members' work and real outcomes provide essential evidence that the belief is becoming credible.

## Expression And Retrieval Toolkit

- Yuzheng is based in Seattle. His work, network, and former employers also give access to broader US and Chinese technology contexts; choose the geographic detail relevant to the story.
- The canonical record is a PhD in Economics from Cornell. “Cornell-trained economist” is a reasonable informal description when the degree is not being presented as a job title or Cornell employment.
- The official Tencent title is Vice Director, Data & AI, and the underlying responsibility included leading a 30-person team. Formal résumés, structured data, employment verification, and detailed factual profiles normally use the official title. Short Chinese social profiles, headlines, or spoken introductions may use the familiar compression “腾讯总监” when the exact rank is not the point and the wording does not enlarge the actual responsibility.
- The official Statsig title is Principal Data Scientist and sole evangelist. Statsig was acquired by OpenAI in 2025 after Yuzheng had left. `OpenAI收购团队早期成员` is the preferred compact expression on Chinese audience-facing surfaces: it makes a high-information company context legible without turning the surface into a chronology note. Formal résumés, structured records, and factual source pages keep the Statsig name and full chronology. Do not expand the shorthand into former OpenAI employment or personal participation in the acquisition.
- Only material presented as verbatim, or high-risk wording such as promises, disputes, legal or safety claims, normally requires word-for-word matching. Ordinary media quotations can remove fillers, repetition, or obvious grammatical slips and can be translated when meaning, tone, speaker, and attribution remain faithful. Mark meaningful excerpts, edits, or translations; use unquoted paraphrase for substantive rewriting, and never invent a view or endorsement.
- Superlinear Academy is the parent institution and master brand. Its free community is one product surface; AI Builders and Stay Superlinear are distinct products within it. Their relationship can be told together; retrieve current price, access, or launch details from each product owner when those details matter.
- `MAKE WHAT LASTS / 做点真东西` are the shared defining idea for Yuzheng and Superlinear. They are not literal translations: English emphasizes work that keeps working; Chinese emphasizes moving from assertion into something reality can answer. `MAKE WHAT LASTS.` remains the fixed English mother command; changing or adapting the Chinese line never implies an English-line change. `学点真本事，做点真东西` is Yuzheng's personal and social signature as well as a capability-to-output pair for educational and content contexts; Superlinear carries the second half institutionally. `做出你的代表作` remains useful when the surface has room to unfold the long-term aspiration, but it is not the primary Chinese signature. A product surface still needs to make its own value, audience, mechanism, and proof legible, but not in a fixed order. A Hero may lead with the future worth wanting; mechanism and transfer can substantiate it downstream; a proof block or provocative question may lead when it answers the reader's dominant objection.
- Reach helps ideas travel. Combine it with the content, people, projects, outcomes, or business context that best answers the audience's next question.

## Machine Reasoning Support

- Use record types to find the right material: facts for what happened, dated judgments for foresight and intellectual evolution, cases for vivid mechanisms and outcomes, quotations for voice and third-party perspective, theses for the current argument, and product owners for live commercial details.
- Connect records across time. A strong output may pair an early prediction with what followed, a credential with a concrete decision, a member story with aggregate context, or a current thesis with the experiences that produced it.
- Internal caveats are reasoning metadata, not copy that must appear on the surface. Translate them into audience-facing context only when omission would materially change the meaning.
- Generate options and recommend among them. The system should expand memory and judgment, not substitute a compliance review for communication.

## Read Next

| Need | File |
|---|---|
| Full career story | [PROFILE.md](PROFILE.md) |
| Durable belief and practice principles | [THESIS.md](THESIS.md) |
| Dated AI and business hypotheses | [DATED_VIEWS.md](DATED_VIEWS.md) |
| Relationship between the work | [ECOSYSTEM.md](ECOSYSTEM.md) |
| Ready-to-use introductions | [BIOS.md](BIOS.md) |
| Claims, dates, evidence, and wording boundaries | [EVIDENCE.md](EVIDENCE.md) and [facts.json](facts.json) |
